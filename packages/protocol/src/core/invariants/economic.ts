...
export class EconomicInvariants {
  ...
  static calculateTotalObligation(
    ...
    for (const parent of node.parents) {
      ...
      const parentObligation = this.calculateTotalObligation(parent.to, nodes, visited, depth + 1);
      ...
      if (remaining > 0 && node.parents.length > 0) {
        ...
        for (const parent of node.parents) {
          const normalizedShare =
            totalParentShare > 100 ? (parent.share / totalParentShare) * 100 : parent.share;

          const parentAmount = (remaining * normalizedShare) / 100;
