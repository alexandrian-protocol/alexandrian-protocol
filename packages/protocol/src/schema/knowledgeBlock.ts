...
export interface CuratorIdentity {
  ...
  address: string;
  /** KYA stake amount in XANDER tokens */
  stake: bigint;
...
export interface LicenseParams {
  type: KBLicenseType;
  /** Fee in XANDER tokens per query (null if open_access) */
...
export interface ModelCompatibility {
  provider: string;
  model_family: string;
  /** Semver-style version range (e.g. "claude-3-*", "gpt-4-*") */
...
export interface KBQuery {
  ...
  /** Maximum fee willing to pay per query in XANDER */
  max_fee?: bigint;
...
export interface KBQueryResult {
  ...
  relevance: number;
  /** Fee charged for this retrieval in XANDER */
  fee_charged: bigint;
